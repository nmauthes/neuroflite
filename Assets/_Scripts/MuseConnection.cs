...
public class MuseConnection : MonoBehaviour {
...
//        muse.listenForDataPacket("ARTIFACTS");
    ...
    void Update () {
		...
		MuseData museData = MuseData.CreateFromJSON (dataBuffer);

		if (museData.DataPacketType == "ALPHA_RELATIVE") {
			// Get the average relative alpha across each electrode
			float currentAlphaSum = 0;
			foreach (float f in museData.DataPacketValue) {
				currentAlphaSum += f;
			}

			float currentAlpha = currentAlphaSum / museData.DataPacketValue.Length;

			if (alphaSamples.Count > sampleSize) { alphaSamples.RemoveAt (0); }
			alphaSamples.Add (currentAlpha);

			float sum = 0;
			foreach (float f in alphaSamples) {
				sum += f;
			}

			mellowScore = Mathf.Clamp01(sum / alphaSamples.Count * handicap); // Get moving avg of relative alpha score
		}

        // Display the data in the UI Text field
		dataText.text = "Mellow Score: " + mellowScore;
    }

	// Creates a struct from received Muse data
	[System.Serializable]
	public struct MuseData {
		public string DataPacketType;
		public float[] DataPacketValue;
		public int TimeStamp;

		public static MuseData CreateFromJSON(string json) {
			return JsonUtility.FromJson<MuseData> (json);
		}
	}
}
